...
export default function BooksBackground() {
  ...
  const bookCovers =
    data?.item.map((book) => ({ cover: book.cover, isbn: book.isbn13 })) ?? [];

  // 10개씩 나눈 책 커버 배열
  const splitBookCovers = useMemo(
    () => splitArray(bookCovers, 10),
    [bookCovers]
  );
  ...
  return (
    <div className='flex gap-4 justify-center items-center h-screen overflow-hidden relative'>
      {[...Array(10)].map((_, index) => (
        <BookSlider
          key={index}
          books={splitBookCovers[index % splitBookCovers.length] || []}
          direction='vertical'
          reverse={index % 2 !== 0}
        />
      ))}
